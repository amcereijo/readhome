...
- **WHEN** the owner activates `Rotate link` while sharing is enabled
- **THEN** the system replaces the stored token with a new UUID
- **AND** sharing remains enabled
- **AND** the panel shows the new public URL
...
- **WHEN** the owner activates `Disable sharing` while sharing is enabled
...
- **WHEN** the owner activates `Create share link` while sharing is disabled
